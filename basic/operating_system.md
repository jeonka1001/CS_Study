...
## 동기 vs 비동기
> 프로그램의 흐름에 따라 작업을 처리하는 방식 
#### 동기 ?
> 다수의 개체들이 동일하게 무언가를 가지는것, 무언가가 동일하게 하는 것  
> 일련의 흐름 속에서 순서대로 행위가 행해지는 것
###### 장점
- 어떠한 작업이 요청됐을 때 해당 작업의 순서까지 기다린다.
- 작업의 순서가 중요할 때 사용
###### 단점
- 특정 작업이 길어질 경우 전체 작업에 영향을 미친다.

#### 비동기 ? 
> 동기가 아닌 것
> 일련의 흐롬 속에서 흐름 외적으로 순서를 지키지 않고 독립적으로 행위가 행해지는 것
###### 장점
- 어떠한 작업이 요청됐을 때 순서를 기다리지 않고 해당 작업을 수행한다.
- 작업의 순서가 상관 없고, 다른 작업과 독립적으로 수행되며, 해당 작업의 결과가 필요할 때 사용
###### 단점
- 작업이 끝날 때 따로 이벤트를 감지하고 결과를 받는 작업을 해야 하기 때문에 비교적 느릴 수 있다.
