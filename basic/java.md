...
> 추상화, 캡슐화, 상속, 다형성
- 추상화
    - 객체의 공통점을 파악 하나의 집합으로 다루는 것
- 캡슐화
    - 관련있는 변수, 메소드를 하나로 묶어 객체화 하는것
    - 이를 통해 정보 은닉을 할 수 있다.
- 다형성
    - 서로 다른 클래스의 객체가 같은 메시지를 받았을 때 각자의 방식으로 작동하는것 ( 오버라이딩 )
...
- 인터페이스의 모든 변수는 기본적으로 ```public static final``` , 메소드는 ```public abstrach``` 이지만 추상 클래스는 아니다.
...
## Generic
> 타입의 정보를 동적으로 넘겨줄 수 있고, 컴파일 시점에서 타입 체크를 해주는 것
